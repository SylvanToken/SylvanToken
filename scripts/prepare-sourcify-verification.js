...
console.log('\n📦 Preparing files for Sourcify verification...\n');

// Create verification directory
const verifyDir = path.join(__dirname, '../sourcify-verification');
...
filesToCopy.forEach(file => {
    ...
    const destPath = path.join(verifyDir, file);
...
// Copy metadata
const metadataPath = path.join(__dirname, '../artifacts/contracts/SylvanToken.sol/SylvanToken.json');
const metadataDestPath = path.join(verifyDir, 'metadata.json');
fs.copyFileSync(metadataPath, metadataDestPath);
console.log('✅ metadata.json');

// Create README
const readme = `# Sourcify Verification Files
...
fs.writeFileSync(path.join(verifyDir, 'README.md'), readme);
console.log('✅ README.md');

console.log('\n✅ All files prepared!');
console.log('\n📁 Verification files location:');
console.log(verifyDir);
console.log('\n🔗 Next steps:');
console.log('1. Go to: https://sourcify.dev/#/verifier');
console.log('2. Upload files from:', verifyDir);
console.log('3. Verify contract');
console.log('\nOR use BSCScan directly with the guide in BSC_VERIFICATION_GUIDE.md\n');
