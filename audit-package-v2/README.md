...
- 👥 [Holder List](https://bscscan.com/token/0xc66404C3fa3E01378027b4A4411812D3a8D458F5#balances)
...
For enhanced security, Sylvan Token supports separating the deployer wallet (which deploys the contract) from the owner wallet (which has administrative privileges).

**Quick Start:**

```bash
# Set environment variables
export DEPLOYER_ADDRESS=0x...  # Standard wallet for deployment
export OWNER_ADDRESS=0x...     # Hardware wallet or multisig for admin control
...
**Why Separate Roles?**
- **Security**: Use a hardware wallet or multisig for owner without exposing it during deployment
- **Best Practice**: Deployer pays gas fees, owner controls admin functions
- **Flexibility**: Change ownership without redeployment

**Learn More:**
- [Ownership Transfer Guide](./docs/OWNERSHIP_TRANSFER_GUIDE.md) - Complete guide to role separation
- [Hardware Wallet Integration](./docs/HARDWARE_WALLET_INTEGRATION_GUIDE.md) - Using Ledger/Trezor as owner
- [Multisig Setup](./docs/MULTISIG_WALLET_INTEGRATION_GUIDE.md) - Using Gnosis Safe as owner
...
└── libraries/                    # Modular libraries
    ...
    └── MultiSigPauseManager.sol # Decentralized pause
```
...
#### 2.5 **Multi-Signature Pause Mechanism**
The contract features a decentralized pause system that eliminates single-owner control:

- **Quorum-Based**: Requires 2-10 authorized signers to approve pause/unpause
...
- **Auto-Unpause**: Contract automatically unpauses after 7-30 days
...
This addresses the centralization risk where a single owner could lock all token transfers.

#### 3. **Security Features**
- ReentrancyGuard on all state-changing functions
- Ownable access control with transfer mechanisms
- Comprehensive input validation
- Multi-signature pause mechanism (2-10 signers required)
- Timelock protection for pause operations (6-48 hours)
- Automatic unpause after max duration (7-30 days)
...
| Multi-Sig Pause | Decentralized control | ✅ Active |
| Timelock Protection | 6-48 hour delays | ✅ Active |
| Auto-Unpause | 7-30 day max duration | ✅ Active |
...
- [Multi-Sig Pause Guide](./docs/MULTISIG_PAUSE_GUIDE.md) - Decentralized pause mechanism
